...
The platform should never assume that one company uses the same IPAM, CMDB, EMS, monitoring platform, credential vault, cloud provider, or automation stack as another company. Large service-provider environments are heterogeneous by default.

Therefore, Truthwatcher must expose clear integration boundaries so users can bring their own systems into the discovery and modeling workflow without changing the core product.
...
Truthwatcher should integrate with these systems through connectors, adapters, importers, exporters, and plugins — not by baking vendor-specific assumptions into the kernel.

The rule is:

> Stable core, replaceable integrations.

If a feature only makes sense for one vendor, one company, one IPAM, one NMS, or one EMS, it belongs behind an adapter boundary.
