# Steering Docs

This folder contains Truthwatcher project steering, guardrail, roadmap, and prompt-pack documents.

Root-level Markdown is intentionally kept small:

- `README.md` for project overview and quickstart.
- `CONTRIBUTING.md` for contributor workflow.

## Core Project Documents

- [Project Charter](PROJECT_TRUTHWATCHER.md)
- [Agent Collaboration Contract](AGENT_COLLABORATION_CONTRACT.md)
- [Architecture Decisions](ARCHITECTURE_DECISIONS.md)
- [Data Model](DATA_MODEL.md)
- [Safety Model](SAFETY_MODEL.md)
- [MVP Spec](MVP_SPEC.md)
- [Extensibility Model](EXTENSIBILITY_MODEL.md)
- [Roadmap](ROADMAP.md)

## Agent And Prompt-Pack Documents

- [Agent Instructions](AGENT_INSTRUCTIONS.md)
- [Coding Session Template](CODING_SESSION_TEMPLATE.md)
- [Prompt Index](PROMPT_INDEX.md)
- [Prompt Pack README](README_PROMPT_PACK.md)
- [Start Here Codex Prompt](START_HERE_CODEX_PROMPT.md)

## Legacy / Alternate Charter

- [Project Charter Alternate](PROJECT_CHARTER.md)
