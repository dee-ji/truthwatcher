...
Task: prepare the SSH collector boundary without making it the main workflow yet.

Deliverables:
1. Define SSH collector config:
   - target host/IP
   - port
   - username
   - credential reference or environment password for local dev
   - platform hint
   - timeout
2. Implement SSH collector behind the same Collector interface.
3. Use a well-known Go SSH library and document the decision.
4. Ensure every command is checked by policy before execution.
5. Add clear warnings that SSH collector is read-only and command allowlisted.
6. Add integration-test placeholders but do not require live devices in normal tests.

Constraints:
- No credential guessing.
- No brute force.
- No config commands.
- No terminal server support yet.
